  ...
  const updateDOMRefs = useCallback((data: PanelData, event: TimeEvent) => {
    ...
    Object.entries(refInputs.current).forEach(([key, ref], i) => {
      if (ref !== null) {
        ...
        if (refIdxs.current[input_field_key] === undefined || refIdxs.current[input_field_key]! >= timeValues.length - 1) {
    ...
    requestAnimationFrame(() => {
      if (cesiumViewer !== undefined) {
        ...
        if (JulianDate.greaterThanOrEquals(newTime, cesiumViewer.clock.startTime) && JulianDate.lessThan(newTime, cesiumViewer.clock.stopTime)) {
