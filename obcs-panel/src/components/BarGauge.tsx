...
const barDimensions = (values: Vector<any>, valueIdx: number, labelIdx: number, orientation: number, xMax: number,  yMax: number,  barScale: any) => {
    ...
    const subscriber = eventBus.getStream(TimeEvent).subscribe(event => {
      if (event.payload.time !== undefined) {
        //refUnixTime.current = event.payload.time;
          ...
          const {x, y, barWidth, barHeight} = barDimensions(data.series[bidx].fields[1].values, refIdx.current, 1, orientation, xMax, yMax, barScale);
    ...
    <svg width={xMax} height={yMax}>
      <Group>
        {[1].map((val: number, i: number) => {
          ...
          const {x, y, barWidth, barHeight, graphWidth, graphHeight} = barDimensions(data.series[bidx].fields[val].values, data.series[bidx].fields[val].values.length-1, val, orientation, xMax, yMax, barScale);
          return (
            <Group key={`bar-${val}`}>
              <rect
                x={x}
