...
const ColoredPanels = (refInputs: React.MutableRefObject<RefBcreg[]>) => {
  return (
    <div style={{
      display: 'grid',
      ...
      {refInputs.current.map((bcregRef, i) => {
        return (
          <HorizontalGroup key={`bcreg-${i}`}>
            <text >{`BCREG TEMP ${i + 1}`}</text>
            <input ref={(ref) => (bcregRef['temp'] = ref)} style={tempStyle} type="text" value="" />
          </HorizontalGroup>
        )
      })}
      <VerticalGroup>
