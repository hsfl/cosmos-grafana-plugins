...
import { EventBus, PanelData, SelectableValue } from '@grafana/data';
...
export const useDomUpdate = (cpu: SelectableValue<string> | undefined): DomUpdateReturn => {
      ...
      Object.entries(refInputs.current).forEach(([key, ref]) => {
        if (ref !== null) {
          ...
          for (; refIdxs.current[input_field_key]! < timeValues.length - 1; refIdxs.current[input_field_key]!++) {
            ...
            if (time > event.payload.time!) {
              if (refIdxs.current[input_field_key]! > 0) {
                refIdxs.current[input_field_key]! -= 1;
              }
              break;
            }
          }
          // Column name and cpu name must match
          const field = data.series[0].fields.find((field) => field.name === key && field?.labels?.name === cpu?.label);
          if (field === undefined) {
            return;
          }
